...
TRAIN_STEPS=122000 # e.g. llama: 1T tokens / 4M tokens_per_batch = 250000 steps
...
if [[ -f "${CHECKPOINT_SAVE_DIR}/latest_checkpointed_iteration.txt" ]]; then
  # resume training
  CHECKPOINT_ARGS="--load ${CHECKPOINT_SAVE_DIR} --override-opt_param-scheduler --no-load-optim"
else
  # first training
  CHECKPOINT_ARGS="--load ${CHECKPOINT_DIR} --no-load-rng --no-load-optim"
fi

# 2epoch目の開始時のみ'CHECKPOINT_ARGS'に--override-opt_param-scheduler,--no-load-optimを指定し、再開時は外す
# 2epoch目の開始時に1epoch終了時点のlr schedulerをリセットするため
