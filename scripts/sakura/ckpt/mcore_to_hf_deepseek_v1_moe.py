            ...
            for key, value in list(state_dict['model'].items()):
                if 'linear_fc' in key and 'weight' in key and not '_extra_state' in key:
                    ...
                    if 'shared_experts' in key:
                        # Shared experts are not divided by EP rank, so they are added as is
                        new_key = key
                    else:
...
def convert_checkpoint_from_megatron_to_transformers(args):
    ...
    config.bos_token_id = 1  # llmjp models
    config.eos_token_id = 2  # llmjp models
    ...
    config.moe_intermediate_size = megatron_args.ffn_hidden_size
    config.moe_layer_freq = 1
    config.n_routed_experts = megatron_args.num_experts

    shared_expert_size = megatron_args.moe_shared_expert_intermediate_size
    ffn_size = megatron_args.ffn_hidden_size
    assert (
        shared_expert_size % ffn_size == 0
    ), f"moe_shared_expert_intermediate_size ({shared_expert_size}) must be divisible by ffn_hidden_size ({ffn_size})"
    config.n_shared_experts = shared_expert_size // ffn_size
    config.norm_topk_prob = False if megatron_args.moe_router_pre_softmax else True
    config.num_attention_heads = megatron_args.num_attention_heads
    config.num_experts_per_tok = megatron_args.moe_router_topk
    ...
    for pp_rank in range(pp_size):
        ...
        for key, val in get_element_from_dict_by_path(tp_state_dicts[0], path).items():
            ...
            if 'linear_fc' in key:
                if 'shared_experts' in key:
                    print(f"Processing shared expert key: {key}")
                    key_list = key.split('.')
