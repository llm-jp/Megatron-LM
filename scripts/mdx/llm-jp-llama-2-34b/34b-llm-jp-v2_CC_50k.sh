...
CHECKPOINT_SAVE_DIR=/data/checkpoints_34b/CC_v2_code10K_en20K_ja30K_ver2.2
...
# job name
JOB_NAME="34b-${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-DP=${DATA_PARALLEL_SIZE}-TP=${TENSOR_PARALLEL_SIZE}-PP=${PIPELINE_PARALLEL_SIZE}-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}"
...
/usr/local/openmpi-4.1.5/bin/mpirun -np $NUM_GPUS \
  ...
  --use-mpi \
  --wandb-name ${JOB_NAME} \
  --wandb-project "megatron-lm-34B-2023-0111" \
  --wandb-entity "llm-jp"
