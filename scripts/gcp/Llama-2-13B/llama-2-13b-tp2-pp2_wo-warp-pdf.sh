#!/bin/bash
#SBATCH --job-name=llama-2-13Bv3-exp1-wo_warp
#SBATCH --partition=a3
#SBATCH --exclusive
#SBATCH --nodes 8
...
source /home/ext_taishi_nakamura_rio_gsic_tit/workspace/Megatron-LM/venv/bin/activate
...
CHECKPOINT_SAVE_DIR=/home/ext_yuma_tsuta_nii_ac_jp/checkpoints/13B-exp1-wo_warp_pdf/tp${TENSOR_PARALLEL_SIZE}-pp${PIPELINE_PARALLEL_SIZE}-ct${CONTEXT_PARALLEL_SIZE}
...
# job name
JOB_NAME="13Bv3-exp1-wo_warp_pdf-${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-DP=${DATA_PARALLEL_SIZE}-TP=${TENSOR_PARALLEL_SIZE}-PP=${PIPELINE_PARALLEL_SIZE}-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}-z-loss-overlap-param-gather-grad-reduce"
...
mpirun -np $NUM_GPUS \
  ...
  -x PATH \
  python /home/ext_taishi_nakamura_rio_gsic_tit/workspace/Megatron-LM/pretrain_gpt.py \
  ...
  --log-throughput \
  --wandb-name ${JOB_NAME} \
  --wandb-project "13B-exp1-wo_warp_pdf" \
  --wandb-entity "llm-jp" \
  --use-gcp-dynamic-checkpointing \
  --data-cache-path /home/ext_yuma_tsuta_nii_ac_jp/data_cache
