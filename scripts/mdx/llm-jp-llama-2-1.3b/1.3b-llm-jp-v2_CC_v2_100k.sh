...
CHECKPOINT_SAVE_DIR=/model/checkpoints_1.3b/CC_v2_code20K_en40K_ja60K_ver2.2
...
# job name
JOB_NAME="1.3b-${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-DP=${DATA_PARALLEL_SIZE}-TP=${TENSOR_PARALLEL_SIZE}-PP=${PIPELINE_PARALLEL_SIZE}-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}"
...
/usr/local/openmpi-4.1.5/bin/mpirun -np $NUM_GPUS \
  ...
  --use-mpi \
  --wandb-name ${JOB_NAME} \
  --wandb-project "megatron-lm-1.3B" \
  --wandb-entity "llm-jp"
