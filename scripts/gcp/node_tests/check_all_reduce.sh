#!/bin/bash
#SBATCH --job-name=check-gpus
#SBATCH --partition=a3
#SBATCH --exclusive
#SBATCH --nodes 2
#SBATCH --gpus-per-node=8
#SBATCH --ntasks-per-node=8
#SBATCH --output=outputs/check-gpus/%x-%j.out
#SBATCH --error=outputs/check-gpus/%x-%j.out
...
mpirun -np $NUM_GPUS \
  ...
  python scripts/gcp/node_tests/check_all_reduce.py
